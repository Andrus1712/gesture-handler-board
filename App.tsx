...
export default function App() {
  ...
  const [points, setPoints] = useState([
    {id: 1, x: useSharedValue(0), y: useSharedValue(0), prevX: useSharedValue(0), prevY: useSharedValue(0)},
    {id: 2, x: useSharedValue(0), y: useSharedValue(0), prevX: useSharedValue(0), prevY: useSharedValue(0)},
    {id: 3, x: useSharedValue(0), y: useSharedValue(0), prevX: useSharedValue(0), prevY: useSharedValue(0)},
    // Add more points if necessary
  ]);
  
  // Store positions to display
  const [positions, setPositions] = useState(
      points.map(point => ({id: point.id, x: 0, y: 0}))
  );
          ...
          {points.map((point) => {
            ...
            return (
                <GestureDetector key={point.id} gesture={createGesture(point)}>
                  <Animated.View style={[animatedStyles, styles.point, {height: pointHeight, width: pointWidth}]}>
